...
using Network.Server.DataProcessing.Friend;
using Network.Server.DataProcessing.Guild;
...
namespace Network.Server.DataProcessing {
    ...
    internal static class RequestManager {
        public static RequestResult Dispatch(IRequest request, ClientInfo client) {
            ...
            FriendRequestManager.Dispatch(dispatcher, client);
        ...
        private static RequestResult SendFriendInvitation(SendFriendInvitationRequest request, ClientInfo client) {
            ...
            if (pending_invitation) {
                var notification_receivers = new List<ClientInfo>();
                var receiver = Server.Data.Clients.Find(p => (p.UserID == request.ReceiverID));

                if (receiver != null) {
                    notification_receivers.Add(receiver);
                }
